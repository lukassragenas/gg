(function ($) {
    ...
    $(window).on('load', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (isJson(response)) {
                        ...
                        if (condition === 'cart_total' || condition === 'cart_totalafter' || condition === 'cart_productspecific' ) {
        ...
        $(document).on('click', '#save_master_settings', function () {
            ...
            var afrsm_sm_count_per_page = $('#afrsm_sm_count_per_page').val();
            if( afrsm_sm_count_per_page > 30 || afrsm_sm_count_per_page <= 0 ){
				alert('Please add 30 or below positive number to apply changes.');
				return false;
			}
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'afrsm_sm_count_per_page': afrsm_sm_count_per_page,
                },
                success: function () {
                    var div = document.createElement('div');
                    div = setAllAttributes(div, {
                    ...
                    setTimeout(function () {
                        $('.ms-msg').remove();
                        document.location.href = coditional_vars.first_path;
                    }, 2000);
                }
            });
        });
